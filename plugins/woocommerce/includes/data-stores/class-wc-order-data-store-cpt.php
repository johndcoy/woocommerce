	...
	protected function get_wp_query_args( $query_vars ) {
		...
		if ( ! empty( $query_vars['post_status'] ) ) {
			if ( is_array( $query_vars['post_status'] ) ) {
				...
				// Keep usable siblings and drop only the offending entries. array_filter() is what
				// decides whether a sibling survives: an inert entry ( '', null, an array ) is
				// reduced to nothing later on and filters no status, so counting it as a survivor
				// would drop the clause entirely and return every order, including trashed ones.
				$query_vars['post_status'] = $usable_statuses;
				$has_unusable_status       = $has_unusable_status && ! array_filter( $usable_statuses );
